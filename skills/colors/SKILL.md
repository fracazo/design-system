...
This skill does not own focus, keyboard, ARIA or live regions (`accessibility`), grouping, logical properties or RTL (`layout`), voice, labels or empty states (`writing`), faces, measure or tabular numerals (`typography`), or radius, shadow and motion (`ui`).

### Status ink pair

On this system, a status wash is not a text colour. `--status-want-ink` is the label on `--status-want-soft`, and `--status-no-ink` is the label on `--status-no-soft`. Measure the rendered pair. Both must clear APCA |Lc| 60 (non-body text) and WCAG 4.5:1 for normal text, in light and dark. The fill (`--status-want`, `--status-no`) stays a dot or a background. `--status-ifnec` has no separate ink: its text uses the brand ink or `--highlight-ink`. Do not put the fill color in the text.
...
Use this format only when the user asks for a standalone color review. When `interface` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
