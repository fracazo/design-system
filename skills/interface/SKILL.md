...
name: interface
description: >-
  User-invoked, cross-discipline interface review that coordinates accessibility, layout, writing, typography, colors, and ui. Use when explicitly invoked for a holistic review of a screen, flow, feature, or product interface. Supports quick and full review modes. Triggers on interface, full interface review, holistic UI audit, cross-discipline design review, review the whole interface.
---
...
This skill owns orchestration only. Accessibility rules belong to `accessibility`; structure to `layout`; copy to `writing`; type to `typography`; color to `colors`; visual polish and motion to `ui`. Never duplicate or override their rules here.
...
This skill owns the final response. When a domain skill is loaded through `interface`, apply its principles and references but ignore its standalone **Review Output Format**. Use the consolidated format, shared severity, and finding cap in this file instead.
...
Each row is one root cause. The **Domain** value is the owning skill name: Accessibility, Layout, Writing, Typography, Colors, or UI. Respect the mode's finding cap. If there are no findings, omit the table and state "No actionable interface findings."
