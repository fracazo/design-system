...
Raw evidence collected by ds-intake from the first product, the second product since 2026-08-25:
...
### 0754afa chore(design): retire the the second product brand file copy

2026-09-05, Alex Fracazo. Files: `src/system/brands/the second product.css`

> Phase 4d: the second product now owns src/system/brands/the second product.css in its
> own repo, so the validated copy here retires and brand:contract holds
> only brands/the first product.css. Nothing imports the removed file; rendering
> is untouched.
> 
> Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>
...
> Tokens no longer live in globals.css: roles.css and brands/the first product.css
...
### 6038c47 feat(design): add the the second product brand file and the status roles

2026-09-05, Alex Fracazo. Files: `src/system/brands/the first product.css`, `src/system/brands/the second product.css`, `src/system/roles.css`

> SPEC_020, phase 3c of the white-label roadmap.
> 
> brands/the second product.css is the full contract with the second product's real
...
> sister-product tint in the first product rose, dark-brand).
> 
...
> aliases the highlight primitive). brands/the first product.css defines the same
> literals, unused on the first product screens today; PDFs and previews keep
...
> nothing else; swapping the import to brands/the second product.css moves exactly
...
### 8349175 refactor(design): split roles.css from the the first product brand file

2026-09-05, Alex Fracazo. Files: `src/app/globals.css`, `src/system/brands/the first product.css`, `src/system/roles.css`
...
>   brands/the first product.css  every value, light and dark, plus the next/font
...
> public/brand.css, served at the first product.com.au/brand.css, is the public
...
> header documents dark-mode wiring and how a sister product (the second product)
...
> all three PDFs; the second product makes the plan alone). One dialog serves
...
### 84dfd66 feat(analytics): track every click that leads to the program or the second product

2026-09-03, Alex Fracazo. Files: `src/app/feedback/page.tsx`, `src/app/resume/page.tsx`, `src/components/SecondProductLink.tsx`, `src/components/ChatWidget.tsx`, `src/components/Footer.tsx`, `src/components/PlanShortcut.tsx`, `src/components/ProgramLink.tsx`, `src/components/SiteHeader.tsx`, `src/components/landing/OfferingCTA.tsx`, `src/components/mobile-link/ExamplePlanBanner.tsx`, `src/components/mobile-link/MobileLinkPage.tsx`, `src/components/plan/ChangeSomething.tsx`

> cta_clicked now carries ctaDestination (program or the second product) so the two
> paths read apart in GA4 where one location labels both. SecondProductLink
...
Evidence: 8e0c64f (the first product)
...
Destination: exemplar (one commit; becomes a rule if the pattern recurs on the the second product preview)
...
Evidence: 3c9b271, cf0b4fe (design-system); the flagged file is the first product src/components/preview/OutputPreviewTabs.tsx.
Exceptions: none.
...
Rationale: 6038c47 added the status ramp to the contract for the second product and made the first product define six values it does not use; chip-3 in each brand file holds the sister product's colour; dark-3 and dark-4 are one landing component's paint. Alex raised this on 5 Sep as the white-label question.
Evidence: 6038c47, 8349175 (the first product); 3a98e2c (the second product); DESIGN.md brand chapters.
...
- the second product type-role pass still pending; its fluid-type lint stays at warn.

## Next intake

- the first product: next intake runs with `--since dd8495b`
- the second product: next intake runs with `--since 9d63e97`
