...
How copy renders (capitalization via `text-transform`, truncation, smart punctuation) is covered by the `typography` skill; error markup and announcements (`aria-invalid`, live regions) by the `accessibility` skill; room for translated strings by the `layout` skill. Rendered-pair contrast is `colors`. Radius, shadow and motion are `ui`. This skill does not own those domains.
...
### 13. Plain Words, Not Machine-Written Ones

Voice lives in the product's own guide. These patterns mark text as machine-written in any product, and they are findings:

- Hedging and throat-clearing openers: "it's worth noting", "in today's", "as an AI", "great question", "certainly".
- Rhetorical scaffolding: "not just X, but Y", "X isn't Y, it's Z", tricolons used as a default rhythm, a question the text then answers itself.
- Inflated verbs and adjectives: "delve", "leverage", "utilise", "robust", "streamline", "elevate", "harness", "navigate" (when it is not navigation), "landscape", "tapestry", "journey". Also "seamless", "empower", "unlock", "cutting-edge", "revolutionise".
- Agent jargon in product copy: "agentic", "orchestrate", "surface" as a verb, "spin up".
- Closing offers and sign-offs: "let me know if", "I hope this helps", "feel free to".
- Emoji in interface copy or as list markers. Exclamation marks outside genuinely celebratory copy.
...
| "It's worth noting that this is seamless" | Say the fact in plain words |
| `"You have " + n + " messages"` | Full templated strings with pluralization |

## Review Output Format

Use this format only when the user asks for a standalone writing review. When `interface` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
