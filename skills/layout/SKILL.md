...
Hit-area sizes and focus behavior are covered by the `accessibility` skill; visual polish (radius, shadows, animation) by the `ui` skill; line length and text spacing by the `typography` skill. Voice and labels are `writing`. Rendered-pair contrast is `colors`. This skill does not own those domains.
...
Use this format only when the user asks for a standalone layout review. When `interface` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
