...
Typography (text wrapping, font rendering, tabular numbers, spacing) is covered by the `typography` skill; use that for anything text-related. Accessibility (hit areas, focus states, keyboard support, ARIA, reduced motion) is covered by the `accessibility` skill. Layout structure (grouping, spacing between sections, breakpoints, spatial RTL) is covered by the `layout` skill. Voice, labels and empty states are `writing`. Rendered-pair contrast is `colors`. This skill does not own those domains.
...
Use this format only when the user asks for a standalone UI-polish review. When `interface` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
