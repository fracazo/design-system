...
The words themselves (button labels, error messages, empty states) are covered by the `writing` skill; semantic heading structure by `accessibility`; spatial RTL layout and logical CSS properties by `layout`; rendered-pair contrast measurement and color remediation by `colors`. Radius, shadow and motion are `ui`. This skill does not own those domains. It owns how text renders, wraps, and behaves in mixed-direction content.
...
Store text in natural case and control presentation with `text-transform`, so redesigns never require rewriting copy. Use smart punctuation: curly quotes in prose (straight quotes in code), an en dash for ranges like `2010–2020`, the single ellipsis character, `&nbsp;` to keep values like `16 px` together and `&shy;` to control where long words may break. This system bans the em dash: use a comma, colon, full stop or parentheses.
...
Use this format only when the user asks for a standalone typography review. When `interface` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
