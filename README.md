...
It ships nine lint rules, twenty components with intent docs, a brand contract (core plus extensions), four bins, and eight agent skills. You import the roles, then exactly one brand file. Both people and AI assistants build from the same reference.
...
| [`@fracazo/design-system/ui/*`](src/ui) | Twenty components (seventeen shadcn-based, plus house OfferCard, ArticleCard and Flowly), each with intent JSDoc |
...
Twenty typed React components. Import from a per-component subpath so each one's client boundary stays where it declares it.
...
| Directory | Purpose |
|---|---|
| `src/` | `cn` and the twenty components |
