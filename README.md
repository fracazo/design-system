...
**Components.** Each of the nineteen components carries its own guidance in JSDoc: use for, avoid when, variants. The decision sits where it gets made, not in a doc nobody opens.
...
| `@fracazo/design-system` and `./ui/*` | `cn` and nineteen components (button, card, dialog, form, select, sortable-list, offer-card, article-card and the rest), each with intent JSDoc: use for, avoid when, variants |
